...
import edu.API.deserializers.*;
import edu.API.entities.*;
...
public class Loader {
    ...
    public static User loadUsersInfo(){
        User user = new User();
        try {
            String jsonString = loadContent("UsersInformation");//читаем JSON из файла в формате строки

            Gson gson = new GsonBuilder()
                    .registerTypeAdapter(User.class, new UserDeserializer())//регистрируем десериализатор для треков
                    .create();
